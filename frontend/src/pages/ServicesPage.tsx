...
const ServicesPage = () => {
  ...
  useEffect(() => {
    const detailSection = document.getElementById('services-detail');
    if (!detailSection) return;
    const offset = 120;
    const top = detailSection.getBoundingClientRect().top + window.pageYOffset - offset;
  ...
  useEffect(() => {
    ...
    const exists = services.some((service) => service.id === hashId);
    if (exists) {
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-dark-bg' : 'bg-white'}`}>
      ...
      <div className="pt-24 lg:pt-32">
        <div className="container mx-auto px-6 py-16 lg:py-20">
          ...
          <div id="services-detail" className="flex flex-col lg:flex-row gap-8">
            {/* Sidebar Navigation */}
            <aside
