    ...
    <>
      <div className={`fixed top-0 left-0 right-0 z-50 ${
        ...
        <div className="container mx-auto px-4 lg:px-6">
          <div
            className="flex justify-center gap-2 py-3 sm:gap-3 md:gap-4 lg:gap-6"
          >
            {topNavItems.map(({ label, to, icon: Icon, featured }) => (
              <Link
                key={label}
                to={to}
                className={`flex items-center justify-center rounded-2xl w-[60px] h-11 sm:w-[70px] lg:w-[78px] transition-all duration-200 border shadow-sm ${
                  featured
                    ? theme === 'dark'
